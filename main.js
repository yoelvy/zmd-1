...
urlButton: {
displayText: '🧸TIKTOK',
url: 'https://vt.tiktok.com/ZSdkn3LBQ/'
}
}, 
{
urlButton: {
...
Welcome = await getBuffer(`https://hardianto.xyz/api/welcome3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/38dfc6649ab96b7dc7d41.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
Left = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/38dfc6649ab96b7dc7d41.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
                if (anu.action == 'add') {
                    kirbotz.send5Loc(anu.id, cangton, `Name Grup ${metadata.subject} `, Welcome, btn)
                } else if (anu.action == 'remove') {
                   kirbotz.send5Loc(anu.id, cangtion, `Leaving To  ${metadata.subject} `, Left, bton)
                }
            }
        } catch (err) {
            console.log(err)
